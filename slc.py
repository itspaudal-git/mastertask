...
FACILITY_HOURS = {"Westvala": {"min": 0.0, "mid": 10.0, "max": 12.0}}
...
def generate_facility_sheets(_facility, facility_network, _spreadsheetInfo, _requests, _data, _service, _spreadsheet_id, _admin_users, _plannedFacilityEqSheetID, _plannedFacilityWorkCenterTimeStudySheetID, _masterSheetColumns):
	...
	_requests = gs.compileRepeatCellRequest(actualFacilityEqSheetID, _requests, gs.A1NotationToRC("C2:"+ eqListHeaderColumnEnd + str(len(vals_equip))), _service, _spreadsheet_id, "=sumifs('Master Task List'!$" + _masterSheetColumns["expected_duration"] + "$6:$" + _masterSheetColumns["expected_duration"] + "$1200,\
			'Master Task List'!$" + _masterSheetColumns["equipment"][0] + "$6:$" + _masterSheetColumns["equipment"][0] + "$1200, \"=\"&$A2,'Master Task List'!$" + _masterSheetColumns["facility"] + "$6:$" + _masterSheetColumns["facility"] + "$1200, \"=" + _facility.name.capitalize() + "\",'Master Task List'!$" + _masterSheetColumns["day_allocated"] + "$6:$" + _masterSheetColumns["day_allocated"] + "$1200,\"=\"&C$1)+sumifs('Master Task List'!$" + _masterSheetColumns["expected_duration"] + "$6:$" + _masterSheetColumns["expected_duration"] + "$1200,'Master Task List'!$" + _masterSheetColumns["equipment"][1] + "$6:$" + _masterSheetColumns["equipment"][1] + "$1200, \"=\"&$A2,\
			'Master Task List'!$" + _masterSheetColumns["facility"] + "$6:$" + _masterSheetColumns["facility"] + "$1200, \"=" + _facility.name.capitalize() + "\",'Master Task List'!$" + _masterSheetColumns["day_allocated"] + "$6:$" + _masterSheetColumns["day_allocated"] + "$1200,\"=\"&C$1)+sumifs('Master Task List'!$" + _masterSheetColumns["expected_duration"] + "$6:$" + _masterSheetColumns["expected_duration"] + "$1200,'Master Task List'!$" + _masterSheetColumns["equipment"][2] + "$6:$" + _masterSheetColumns["equipment"][2] + "$1200, \"=\"&$A2,'Master Task List'!$" + _masterSheetColumns["facility"] + "$6:$" + _masterSheetColumns["facility"] + "$1200, \"=" + _facility.name.capitalize() + "\",\
			'Master Task List'!$" + _masterSheetColumns["day_allocated"] + "$6:$" + _masterSheetColumns["day_allocated"] + "$1200,\"=\"&C$1)+sumifs('Master Task List'!$" + _masterSheetColumns["expected_duration"] + "$6:$" + _masterSheetColumns["expected_duration"] + "$1200,'Master Task List'!$" + _masterSheetColumns["equipment"][3] + "$6:$" + _masterSheetColumns["equipment"][3] + "$1200, \"=\"&$A2,'Master Task List'!$" + _masterSheetColumns["facility"] + "$6:$" + _masterSheetColumns["facility"] + "$1200, \"=" + _facility.name.capitalize() + "\",'Master Task List'!$" + _masterSheetColumns["day_allocated"] + "$6:$" + _masterSheetColumns["day_allocated"] + "$1200,\"=\"&C$1)+\
			sumifs('Master Task List'!$" + _masterSheetColumns["expected_duration"] + "$6:$" + _masterSheetColumns["expected_duration"] + "$1200,'Master Task List'!$" + _masterSheetColumns["equipment"][4] + "$6:$" + _masterSheetColumns["equipment"][4] + "$1200, \"=\"&$A2,'Master Task List'!$" + _masterSheetColumns["facility"] + "$6:$" + _masterSheetColumns["facility"] + "$1200, \"=" + _facility.name.capitalize() + "\",'Master Task List'!$" + _masterSheetColumns["day_allocated"] + "$6:$" + _masterSheetColumns["day_allocated"] + "$1200,\"=\"&C$1)")
...
def generate_master_tab(facility_network, _tasks, _spreadsheetInfo, _requests, _data, _service, _spreadsheet_id, _sheetIDs, _masterSheetColumns, _master_columns_sorted, _tovala_subtype_info_list, _planning_users, _mod_plus_users, _execution_users, _productionRecord, _lenTaskCompatibilityList = 26, _maxWorkCenterListColumns = 26, _verbosity = VL_WARNING):
	...
	#_requests = gs.compileDataValidationV1(masterTaskSheetId, _requests, gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Work Center")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Work Center")) + "1200"), _service, _spreadsheet_id, ["='Master Task List'!$" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Work Centers Possible_0")) + "6:$" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Work Centers Possible_3")) + "6"], dv_type = "ONE_OF_RANGE", strict = False)
	#^^ this request can't be completed through the API and must be completed through a template instead

	_requests = gs.compileDataValidationV1(masterTaskSheetId, _requests, gs.A1NotationToRC(_masterSheetColumns['facility'] + "6:" + _masterSheetColumns['facility'] + "1200"), _service, _spreadsheet_id, [facility.name.capitalize() for facility in facility_network.facilityList], dv_type = "ONE_OF_LIST", strict = False)
	_requests = gs.compileRepeatCellRequest(masterTaskSheetId, _requests, gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Equipment Type_0")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Equipment Type_4")) + "1200"), _service, _spreadsheet_id, "=ifna(vlookup(" + _masterSheetColumns['equipment'][0] + "6,indirect($" + _masterSheetColumns['facility'] + "6&\" Equipment List!A2:B70\"),2,FALSE),\"\")")
	...
	_requests = gs.compileWrapStrategy(masterTaskSheetId, _requests, gs.A1NotationToRC(_masterSheetColumns['work_order'] + "6:" + _masterSheetColumns['work_order'] + "1200"), _service, _spreadsheet_id, wrapStrategy = "WRAP")
	_requests = gs.compileWrapStrategy(masterTaskSheetId, _requests, gs.A1NotationToRC("B6:B1200"), _service, _spreadsheet_id, wrapStrategy = "CLIP")

	_requests = gs.compileBordersBox(masterTaskSheetId, _requests, gs.A1NotationToRC("A6:E1200"), _service, _spreadsheet_id)
	_requests = gs.compileBordersBox(masterTaskSheetId, _requests, gs.A1NotationToRC("G6:L1200"), _service, _spreadsheet_id)
	_requests = gs.compileBatchColorCellRangeColorV1(masterTaskSheetId, _requests, gs.A1NotationToRC("G6:L1200"), _service, _spreadsheet_id, bgColor = gs.COLOR_YELLOW)
	_requests = gs.compileBordersBox(masterTaskSheetId, _requests, gs.A1NotationToRC("M6:N1200"), _service, _spreadsheet_id)

	equipment_selected_columns = gs.A1NotationToRC("X6:AB1200")
	...
	_requests = gs.compileBordersBox(masterTaskSheetId, _requests, gs.A1NotationToRC("AH6:" + lastColumn + "1200"), _service, _spreadsheet_id)

	pplAssignedColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("# of People Assigned")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("# of People Assigned")) + "1200")
	...
	estHoursColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Estimated Hours")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Estimated Hours")) + "1200")
	...
	dayAllocatedColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Production Day")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Production Day")) + "1200")
	...
	priorityColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Priority")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Priority")) + "1200")
	...
	_requests = gs.compileFormatCells_NumberFormat(masterTaskSheetId, _requests, gs.A1NotationToRC("AV6:AV1200"), _service, _spreadsheet_id, numFormat = "DATE")

	statusColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "1200")
	...
	prodColumns = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Produced Quantity")) + "1200")
	...
	initialsColumn = gs.A1NotationToRC(next(column['column'] for column in _master_columns_sorted if column['title'] == ("Initials")) + "6:" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Initials")) + "1200")
	...
	_requests = gs.compileConditionalFormattingBgColor(masterTaskSheetId, _requests, gs.A1NotationToRC("A6:AZ1200"), _service, _spreadsheet_id, "=$" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "6=\"Complete\"", gs.COLOR_LIGHT_GREEN)
	_requests = gs.compileConditionalFormattingBgColor(masterTaskSheetId, _requests, gs.A1NotationToRC("A6:AZ1200"), _service, _spreadsheet_id, "=$" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "6=\"In Progress\"", gs.COLOR_LIGHT_YELLOW_3)
	_requests = gs.compileConditionalFormattingBgColor(masterTaskSheetId, _requests, gs.A1NotationToRC("A6:AZ1200"), _service, _spreadsheet_id, "=$" + next(column['column'] for column in _master_columns_sorted if column['title'] == ("Status")) + "6=\"Short\"", gs.COLOR_LIGHT_RED)
	...
	_requests = gs.compileBatchColorCellRangeColorV1(masterTaskSheetId, _requests, gs.A1NotationToRC(time_start_column_letters + "6:" + team_column_letters + "1200"), _service, _spreadsheet_id, bgColor = gs.COLOR_YELLOW)
	_requests = gs.compileBatchColorCellRangeColorV1(masterTaskSheetId, _requests, gs.A1NotationToRC(time_end_column_letters + "6:" + time_end_column_letters + "1200"), _service, _spreadsheet_id, bgColor = gs.COLOR_LIGHT_BLUE)
	_requests = gs.compileBordersBox(masterTaskSheetId, _requests, gs.A1NotationToRC(time_end_column_letters + "6:" + time_end_column_letters + "1200"), _service, _spreadsheet_id)
	_requests = gs.compileRepeatCellRequest(masterTaskSheetId, _requests, gs.A1NotationToRC(time_end_column_letters + "6:" + time_end_column_letters + "1200"), _service, _spreadsheet_id, "=if(isblank(" + time_start_column_letters + "6),\"\"," + time_start_column_letters + "6+" + time_breaks_column_letters + "6/(24*60)+" + _masterSheetColumns['estHours'] + "6/24)")

	_requests = gs.compileWrapStrategy(masterTaskSheetId, _requests, gs.A1NotationToRC(team_column_letters + "6:" + team_column_letters + "1200"), _service, _spreadsheet_id, wrapStrategy = "WRAP")
	_requests = gs.compileChangeColumnWidth(masterTaskSheetId, _requests, gs.A1NotationToRC(team_column_letters + "6:" + team_column_letters + "1200"), _service, _spreadsheet_id, pixelWidth = 230)

	_requests = gs.compileRepeatCellRequest(masterTaskSheetId, _requests, gs.A1NotationToRC(_masterSheetColumns['composite'] + "6:" + _masterSheetColumns['composite'] + "1200"), _service, _spreadsheet_id, "=textjoin(\"-\", FALSE, " + _masterSheetColumns["work_order"] + "6, " + _masterSheetColumns["day_allocated"] + "6, " + _masterSheetColumns["meals"] + "6,\"Cycle\"," + _masterSheetColumns["cycle"] + "6)")


	short_surplus_column_letters = next(column['column'] for column in _master_columns_sorted if column['title'] == ("Short/Surplus"))
	_requests = gs.compileBatchColorCellRangeColorV1(masterTaskSheetId, _requests, gs.A1NotationToRC(short_surplus_column_letters + "6:" + short_surplus_column_letters + "1200"), _service, _spreadsheet_id, bgColor = gs.COLOR_LIGHT_GREEN)
	_requests = gs.compileConditionalFormattingBgColor(masterTaskSheetId, _requests, gs.A1NotationToRC(short_surplus_column_letters + "6:" + short_surplus_column_letters + "1200"), _service, _spreadsheet_id, "=$" + short_surplus_column_letters + "6<0", gs.COLOR_LIGHT_RED)
	_requests = gs.compileRepeatCellRequest(masterTaskSheetId, _requests, gs.A1NotationToRC(short_surplus_column_letters + "6:" + short_surplus_column_letters + "1200"), _service, _spreadsheet_id, "=AY6-M6")
	...
	_data.append({
			"range": "Master Task List!A6:G1200",
			"values": tasks_0,
			"majorDimension": "ROWS"
		})

	_data.append({
			"range": "Master Task List!L6:N1200",
			"values": tasks_1,
			"majorDimension": "ROWS"
		})

	_data.append({
			"range": "Master Task List!X6:AB1200",
			"values": tasks_2,
			"majorDimension": "ROWS"
		})

	_data.append({
			"range": "Master Task List!AH6:" + lastColumn + "1200",
			"values": tasks_3,
			"majorDimension": "ROWS"
		})
		...
		for meal in meals:
			...
			if meal['totalMeals']>0:
				...
				if(meal['partID'] in [item['partID'] for item in partVersionProcessList['sleeving']]):
					...
					tasks.append(TKS.Task(cycle, sleeving_config + " _Sleeving (Meal " + str(meal['mealCode']) + ")", 'Sleeving', next(item['process_list'] for item in partVersionProcessList['sleeving'] if item['partID'] == meal['partID']), _inputs = [], _outputs = [TKS.Task_I_O(meal['totalMeals']*multiplier,'Meals',"")], _process_yield = 1, _planned_location = facility, _planned_day = DAYS[next(day[0] for day in enumerate(DAYS) if day[1] == CYCLE_SHIP_DAY_MAP[cycle])-2], _meals = [meal['mealCode']]))
				else:
					tasks.append(TKS.Task(cycle, sleeving_config + " _Sleeving (Meal " + str(meal['mealCode']) + ")", 'Sleeving', 'Sleeving: (Default)', _inputs = [], _outputs = [TKS.Task_I_O(meal['totalMeals']*multiplier,'Meals',"")], _process_yield = 1, _planned_location = facility, _planned_day = DAYS[next(day[0] for day in enumerate(DAYS) if day[1] == CYCLE_SHIP_DAY_MAP[cycle])-2], _meals = [meal['mealCode']]))
				...
				for component in meal['billOfMaterials']:
					...
					try:
						...
						if container_tag == 'bagged meal extra' or container_tag == 'unsealed body armor':
							tasks.append(TKS.Task(cycle, component['title'] + " [" + str(round(component['qtyPounds']*GRAMS_PER_LB))+" g Armor]", 'Band Sealing','Band Sealing: Breads', _inputs = [TKS.Task_I_O(component['qtyPounds']*GRAMS_PER_LB, 'Grams',component['partID'])], _outputs = [TKS.Task_I_O(meal['totalMeals']*multiplier,\
						...
						if container_tag == "sealed body armor":
							tasks.append(TKS.Task(cycle, component['title'] + " [" + str(round(component['qtyPounds']*GRAMS_PER_LB))+" g]" + " Armor", 'Tray Portioning and Sealing','Tray Portioning and Sealing: (1 Drop)', _inputs = [TKS.Task_I_O(component['qtyPounds']*GRAMS_PER_LB, 'Grams',component['partID'])], _outputs = [TKS.Task_I_O(meal['totalMeals']*multiplier,\
								'Bags',"")], _process_yield = 1, _planned_location = facility, _planned_day = portion_day, _meals = [meal['mealCode']], _parents = [meal['apiMealTitle']], _target_weight = component['qtyPounds']*GRAMS_PER_LB))
							if(_verbosity <= VL_INFORMATIONAL):
								print("Added Band Sealingto %s on %s" % (facility,portion_day))
						...
						if('tray' in container_tag):
							...
							if(not component_portioning_task_created[component_index-1]):
								...
								if container_tag == 'tray 1 unsealed body armor' or container_tag == 'tray 2 unsealed body armor' or container_tag == 'tray 1 bagged body armor' or container_tag == 'tray 2 bagged body armor'  or container_tag == 'tray 1 bagged meal extra' or container_tag == 'tray 2 bagged meal extra':
									...
									component_portioning_task_created[component_index-1] = True
									continue								
								if container_tag == 'tray 1 9x14 bag' or container_tag == 'tray 2 9x14 bag' or container_tag == 'tray 1 14x16 bag' or container_tag == 'tray 2 14x16 bag'  or container_tag == 'tray 1 mushroom bags' or container_tag == 'tray 2 mushroom bags' or container_tag == 'tray 1 veggie bags' or container_tag == 'tray 2 veggie bags':
									tasks.append(TKS.Task(cycle, "Component " + str(component_index) + "| " + component['title'] + "(Armor)", 'Band Sealing','Band Sealing: Breads', _inputs = [TKS.Task_I_O(component['qtyPounds']*GRAMS_PER_LB, 'Grams',component['partID'])], _outputs = [TKS.Task_I_O(meal['totalMeals']*multiplier,\
										'Bags',"")], _process_yield = 1, _planned_location = facility, _planned_day = portion_day, _meals = [meal['mealCode']], _parents = [meal['apiMealTitle']], _target_weight = component['qtyPounds']*GRAMS_PER_LB))
									if(_verbosity <= VL_INFORMATIONAL):
										print("Added Armor Meals: Bandsealing %s on %s" % (facility,portion_day))
									component_portioning_task_created[component_index-1] = True
									continue								
	...
	for facility in facility_network.facilityList:
		for facility_task in facility.facility_tasks:
			tasks.append(TKS.Task(cycles_for_task, facility_task['title'], "Facility","Facility: " + facility_task['subtype'], _inputs = [TKS.Task_I_O(int(facility_task['hours'])*1.0,'Hours',"")], _outputs = [TKS.Task_I_O(0,'Each',"")],\
				 _process_yield = 1, _planned_location = facility.name.capitalize(), _planned_day = facility_task['Day'], _meals = [meals_offered_in_cycle], _parents = [], _target_weight = 0))
...
if __name__ == "__main__":
	...
	[TKS.validTaskTypes,TKS.validTaskSubTypes,TKS.partVersionProcessList,TKS.processSubtypeList] = TKS.loadProcessList("slc_process_list.json")
	#--------------------------------------------------------------------
	...
	#CHICAGO_PRODUCTION_NETWORK = facilities.buildFacilityNetwork("CHICAGO_PRODUCTION_NETWORK_V1.json","equipment_list_V1.json")
	SLC_PRODUCTION_NETWORK = facilities.buildFacilityNetwork("slc_production_network.json","slc_equipment_list.json")
	#--------------------------------------------------------------------
	...
	tasks = compileTasks(SLC_PRODUCTION_RECORD, SLC_PRODUCTION_NETWORK, TKS.partVersionProcessList, scaleMealCount = 0, cycleMethod = "BY_ACTUAL_CYCLE_COUNT", cycleSplit = {"1": 1.0,"2": 0.0})
	for task in tasks:
	...
	# spreadsheet_id = "17vyalOaanu4U792gM30N0679glQJGEDxlaleV67T-Fk"  #Production Schedule Prototype Google Sheet ID
	spreadsheet_id = id_sheet_data_range[0]

	generateTaskListSpreadsheet(SLC_PRODUCTION_NETWORK, SLC_PRODUCTION_RECORD, tasks, SSApp, spreadsheet_id, MASTER_SHEET_COLUMNS_SORTED, TKS.processSubtypeList)